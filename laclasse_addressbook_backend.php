...
class laclasse_addressbook_backend extends rcube_addressbook
{
    ...
    public function __construct($id, $name, $user_data, $profiles_types)
    {
        ...
        if ($this->profil_elv) {
            $this->groupsData = json_decode(interroger_annuaire_ENT(
                ...
                $cfg['laclasse_addressbook_app_id'], $cfg['laclasse_addressbook_api_key'],
                array("structure_id" => $id, "type!" => "GPL", "users.user_id" => $this->user->id, "seenBy" => $this->user->id)));
            ...
            $this->groupsData = json_decode(interroger_annuaire_ENT(
                $cfg['laclasse_addressbook_api_group'],
                $cfg['laclasse_addressbook_app_id'], $cfg['laclasse_addressbook_api_key'],
                array("structure_id" => $id, "type!" => "GPL", "seenBy" => $this->user->id)));
        }
        $this->load_persons();
        $this->ready = true;
    }
    ...
    private function has_intersect_groups($user1, $user2)
    {
    ...
    public function filter_field($all, $search, $field)
    {
        $res = array();
    ...
    public function filter_result($all, $search, $fields)
    {
        if ($fields === '*') {
    ...
    private function array_to_result($all, $page_size = 2000, $list_page = 1)
    {
    ...
    private static function group_cmp($a, $b)
    {
        return strcmp($a["sortname"], $b["sortname"]);
    }

    private static function user_cmp($a, $b)
    {
